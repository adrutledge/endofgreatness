...
- System hide/unhide is driven by events, not hardcoded: an event of type `system_hide` or `system_unhide` toggles a system's visibility for the current date; the `hidden_dates` array becomes a cache derived from past events rather than static data; this allows event chains to reveal or conceal systems dynamically (e.g., a lostech rumor chain uncovers a hidden SLSC facility, or a pirate crackdown removes pirate safe havens)

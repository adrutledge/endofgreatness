...
**Protected entries (`no_downsample: true`)**: some events carry narrative significance (birth of a major character, a unit's founding, a pivotal battle) and must never be condensed. A `no_downsample: true` flag in the event data marks it as protected. During downsizing, protected entries act as barriers — the scan stops before them, and the condensed block covers only non-protected entries preceding the first protected entry. The protected entry remains in place, and the condensed aggregated entry sits immediately before it, maintaining temporal order. The queue discipline is preserved because no entry crosses a protected boundary, and all entries retain their relative order.

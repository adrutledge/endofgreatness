...
- **Organic narrative** (future — post-Phase 6, third-party actors): a single cluster grouping bounties, lostech prospecting, and pirate interference — all systems where the player is not operating under a contract's strict OOB but instead responding to emergent world events
  - **Bounty board**: one-off bounties on specific individuals or unit kills; targets may be generated from forces encountered during active contracts (if a contract involves faction X, a bounty may appear for a notable pilot from the opposing faction); pay flat C-Bill reward plus faction reputation; no duration, transport, salvage, or command rights constraints; can stack with active contracts
  - **Bounties on the player**: enemy factions may place bounties on player personnel or the force as reprisal; when active, contract deployments may attract bounty hunters as a third party hostile to the player (and potentially to the employer); higher-value bounties attract stronger groups; player is not informed by default — must discover through intelligence
  - **Pirate interference**: in systems with pirate activity, pirates may arrive during battle as a third party hostile to both sides; target vulnerable units and withdraw with loot; frequency scales with system instability and player's visible wealth
  - **LosTech rumor tracking**: rumors of Star League caches circulate through events; each rumor tracks location, tech type, reliability, and source; players may investigate by deploying to the rumored location (non-contract deployment); uncovering yields rare components, unique units, or facility upgrades; reliability decays over time; competing factions may follow same rumors
...
- **LosTech rumor events**: LosTech rumors are encountered through strategic events (see Organic Narrative in P3.3) and resolved via non-contract deployment

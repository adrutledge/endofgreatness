...
**Protected entries (`no_downsample: true`)**: some events carry narrative significance (birth of a major character, a unit's founding, a pivotal battle) and must never be condensed. A `no_downsample: true` flag in the event data marks it as protected. During downsizing, the scanner walks from oldest to newest: when it hits a protected entry, it condenses everything before it (up to the previous protected entry or journal start) into a single aggregated diff, then continues scanning from after the protected entry. This produces a journal structure like `[condensed_1][protected_1][condensed_2][protected_2]...[recent_raw_entries]`. Once an entry is marked `"downsampled": true`, it is never re-condensed in a future downsizing — the scanner skips past it. Fragmentation is self-limiting: each condensed block represents one downsample period, protected entries are scattered but individually small, and total journal size stays bounded by the threshold plus the number of protected entries.

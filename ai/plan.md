...
- Gameplay effects of command_rights:
  ...
  - **Liaison**: periodic event popups require player to "request approval" from employer liaison before major actions (planet-side deployment); liaison may refuse, forcing alternative plans; refusal chance decreases with higher reputation; an operational unit assigned to a Liaison contract may not accept a separate side contract simultaneously — once committed to a contract, that OU is locked
...
- **Contract duration**:
  ...
  - Raids: in between emergency and planetary assault in duration, but with wider variance — from a few weeks to several months depending on scope
...
- **Bounty board** (future — non-contract operations): separate from the contract board; offers one-off bounties on specific individuals (defector officers, pirate leaders, enemy aces, criminal cartel heads) or unit kills (specific mech chassis/variant counts); bounty targets may be generated from forces encountered during active contracts — if a contract involves faction X, there's a chance a bounty becomes available for a notable pilot or unit from the opposing faction on that contract's planet; bounties pay a flat C-Bill reward on kill/capture and may include faction reputation bonuses; unlike contracts, bounties carry no duration, transport coverage, salvage rights, or command rights restrictions — the player simply completes the objective and claims the reward; bounties can stack with active contracts (e.g., accept a contract against pirates, pick up a bounty on the pirate leader, collect both)
- **Bounties on the player** (future): enemy factions or aggrieved parties may place bounties on the player's personnel or the entire force as a reprisal for actions during contracts (killing a notable enemy pilot, destroying rare equipment, committing atrocities, breaking a contract); when a bounty is active on the force, contract deployments may attract bounty-hunting mercenaries who enter the battlespace as a third party hostile to the player (and potentially hostile to the employer's forces as well); these bounty hunters are not part of the contract's opposing OOB — they appear specifically to collect, adding unpredictable tactical complications; higher-value bounties attract stronger hunter groups; the player is not informed of bounties placed on them by default (must be discovered through intelligence, intercepted communications, or events)
- **Pirate interference** (future): during any contract in a system with pirate activity (low owner faction control, periphery/rimward systems, failed states), there is a chance pirates may arrive during a battle as a third party hostile to both the player and the contract's opposing force; pirates target vulnerable units (damaged mechs, vehicles, supply caches) and withdraw once they secure sufficient loot; frequency and strength of pirate interference scales with system instability and the player's visible wealth/equipment
- **Contract chains** (future — advanced narrative): contracts may link together to form multi-part story arcs; completing one contract may unlock a follow-up offered by the same employer, escalating in scope and stakes; chains can branch — upon completing a contract, the opposing faction may send an envoy offering to outbid the original employer for the next contract, giving the player a narrative choice that affects faction reputation and future contract availability; chain contracts track persistent NPCs (notable pilots, enemy commanders, employer liaisons) who may reappear across multiple contracts in the chain; branching decisions can lock or unlock future contracts; chains are optional — the player can always decline the follow-up without penalty beyond the missed opportunity
- **Major lore events** (future): contract chains can allow the player to participate in pivotal Inner Sphere conflicts such as the Fourth Succession War (3028–3030), the War of 3039, the Clan Invasion, and other timeline events; these event chains are gated by date (TimeManager advancement) and require the player to be in the relevant region of space; participation in a major event chain may lock out opposing factions for the duration and carries significant reputation and financial consequences; event chains are a special class of contract chain — they may spawn sub-contracts with different employers on the same side, and the player's choices across the chain can affect the outcome (within lore-compatible bounds)
- **Bounties and pirate interference** are grouped under a future **Non-Contract Operations / Third-Party Actors** phase (post-Phase 6), distinct from standard contract generation
...
- **Critical slot limits per location**: per TM rules — Center Torso 12, Side Torso 12, Arms 12, Legs 6, Head 6; vehicle locations differ (turret, front, sides, rear, body)
...
- **Heat sink adequacy**: base 10 free heat sinks from engine; each additional heat sink weighs 1t
  ...
  - **Vehicle heat sources**: ballistic weapons (autocannons, machine guns, Gauss rifles) generate NO heat when mounted on vehicles; check source material for missile weapons (LRMs, SRMs), infantry weapons, and other special cases — do not assume any weapon type generates heat on a vehicle without verification

...
- **Campaign toggles for complexity** (design note): when aerospace, vehicle, and infantry features are implemented, provide campaign-level toggles to disable each domain independently (`aerospace_enabled: bool`, `vehicles_enabled: bool`, `infantry_enabled: bool` in campaign config); when disabled: no contract will require that unit type, no aerospace interdiction/insertion events fire, and the game treats the player's force as mech-only or mech+the enabled types; this allows players who dislike managing aerospace assets, vehicle crews, or infantry platoons to opt out without missing content
